...
// 
// 08.05.24 - задания к лекции 3
//			  Добавлены две лузы в центре длинных стенок стола.
//			  Добавлен атрибут цвета шара.
//			  При создании шаров явно задается их цвет.
// 
#include <iostream>
#include <cmath>
...
public:
	...
	// Если fc == true, то шар считается тем, по которому наносится удар,
	// и он отображается белым цветом (параметр color для него игнорируется).
	Ball(int x, int y, bool fc, Ball* pNextBall, Color color = Color::Blue);
...
Ball::Ball(int x, int y, bool fc, Ball* pNextBall, Color c) :
	pLink(pNextBall),
	fCue(fc)
{
	SetCenter(x, y);
	...
	// Шар, у которого установлен флаг fCue, всегда отображается белым цветом
...
void Ball::Draw() const
{
	switch (color)
	{
		case Color::Blue: glColor3ub(0, 0, 255);     break;	// Синий цвет
		case Color::Green: glColor3ub(0, 255, 0);     break;	// Зеленый цвет 
		case Color::Red: glColor3ub(255, 0, 0);     break;	// Красный цвет 
		case Color::White: glColor3ub(255, 255, 255); break;	// Белый цвет
...
void CreateGlobals()
{
	...
	for (int i = 1; i <= 5; i++)
		for (int j = 1; j <= i; j++)
		{
			...
			Ball* pNewBall = new Ball(190 + i * 8, 100 + 16 * j - 8 * i, false, listOfBalls, c);
